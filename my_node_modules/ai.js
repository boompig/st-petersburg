...
if (require) {
    ...
    var StaticGameData = require("../js/static-data");
}
 ...
 * @param {Array<Card>} upperBoard
 * @param {Array<Card>} lowerBoard
 * @param {number} phase
 * @param {Player} player
 * 
 */
...
AI.generateNextPhaseState = function (state) {
    ...
    const currentDeckIndex = State.phases.indexOf(state.phase);
    const nextDeckIndex = (currentDeckIndex + 1) % State.phases.length;
...
AI.nextPhase = function (phase) {
    let idx = State.phases.indexOf(phase);
    ...
    while (open.length > 0) {
        ...
        for (let i = 0; i < successors.length; i++) {
            ...
            if (visited.has(hashState)) {
                // already visited
            } else if (AI.estimateEvalState(successors[i].state) <= bestScore) {
 ...
 */
AI.analyze = function (state, aiName) {
    return AI.BFS(state);
};
