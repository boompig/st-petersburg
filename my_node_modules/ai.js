        ...
        if (card.type === Card.types.UPGRADE) {
            for (let j = 0; j < player.cards.length; j++) {
                if (player.canUpgradeCard(player.cards[j], card)) {
                    ...
                    newState.currentPlayer.hand.splice(i, 1);
                    newState.currentPlayer.upgradeCard(player.cards[j], card,
                        Card.locations.HAND);
            ...
            if (player.canAffordCard(card, Card.locations.HAND)) {
                newState = state.clone();
                newState.currentPlayer.playCardFromHand(card);
                successors.push({
                    "state": newState,
    ...
    for (i = 0; i < state.upperBoard.length; i++) {
        ...
        if (card.type === Card.types.UPGRADE) {
            ...
            if (player.canAffordCard(card, Card.locations.UPPER_BOARD)) {
                newState = state.clone();
                newState.upperBoard.splice(i, 1);
                newState.currentPlayer.buyCard(card, Card.locations.UPPER_BOARD);
                successors.push({
                    "state": newState,
        ...
        if (player.canPutCardInHand()) {
            newState = state.clone();
            newState.upperBoard.splice(i, 1);
            newState.currentPlayer.putCardInHand(card);
            successors.push({
                "state": newState,
    ...
    for (i = 0; i < state.lowerBoard.length; i++) {
        ...
        if (card.type === Card.types.UPGRADE) {
            ...
            if (player.canAffordCard(card, Card.locations.LOWER_BOARD)) {
                newState = state.clone();
                newState.lowerBoard.splice(i, 1);
                newState.currentPlayer.buyCard(card, Card.locations.LOWER_BOARD);
                successors.push({
                    "state": newState,
 ...
 * - score (number): the estimated number of points this plan will result in
 * - moveList (Array): list of moves that this plan entails
 * - move (Move): the next move to play
 * - numNodes (number): number of nodes visited when planning
 */
...
AI.analyze = function (state) {
    // do this to avoid changing the actual game
    const startState = state.clone();
    return AI.BFS(startState);
};
