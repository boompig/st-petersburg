...
exports.testSinglePossibleMove = function (test) {
    ...
    var correctBestMove = new Move(Move.actions.BUY, Card.locations.UPPER_BOARD,
            cards.market);
    var minScore = cards.market.pointYield;

    test.ok(obj.score >= minScore && obj.score < minScore + 1, "market point yield");
...
exports.testBuyGameEndWorkers = function (test) {
    ...
    test.ok(obj.score > 2 && obj.score < 3, "should have 26 money by the end");
    test.ok(correctBestMove.equals(obj.moveList[0]) &&
...
/***** test upgrade cards *****/
exports.testUpgradeAristocrat = function (test) {
    "use strict";
    var player = new Player();
    player.money = cards.author.cost + cards.pope.cost - 1;

    var upperBoard = [cards.pope];
    var lowerBoard = [cards.author];
    var decks = [9, 9, 9, 9];
    var state = new State(decks, upperBoard, lowerBoard, Card.types.UPGRADE, player);

    var obj = AI.analyze(state);
    var authorMove = new Move(Move.actions.BUY, Card.locations.LOWER_BOARD, cards.author);
    var popeMove = new Move(Move.actions.UPGRADE, Card.locations.LOWER_BOARD,
                    cards.pope, cards.author);
    //for (var i = 0; i < obj.moveList.length; i++)
        //console.log(obj.moveList[i].toString());

    // index 2 because more efficient to buy from bottom level
    test.ok(authorMove.equals(obj.moveList[0]), "buy author first");
    test.ok(popeMove.equals(obj.moveList[2]), "upgrade pope next");
    test.done();
};
