...
AI.estimateEvalState = function (state) {
    // something really crazy, but that cuts some obviously bad states
    // ex. 
    var player = state.player;
...
AI.evalState = function (state) {
    return state.eval();
};

/**
 * Guaranteed to return a value in [0, 1e7)
 */
AI.hashGameState = function (state) {
    var hash = "";
    hash += "D" + state.decks.join(",");
    hash += "T" + state.phase;
    return hash;
};

AI.hashPlayerState = function (state) {
    var player = state.player;
    var hash = "";
    // hash points
    hash += "P" + player.points;
    hash += "C" + player.coins;
    var cardNames = [];
    ...
    cardNames.sort();
    hash += "B" + cardNames.join(",");
    var handNames = [];
    for (var i = 0; i < player.hand.length; i++) {
        handNames.push(player.hand[i].name);
    }
    handNames.sort();
    hash += "H" + handNames.join(",");
    return hash;
};

AI.hashState = function (state) {
    return AI.hashGameState(state) + "#" + AI.hashPlayerState(state);
};

AI.BFS = function (startState) {
    var open = [{ "state": startState, "moveList": [] }];
    var obj, moveList, state, newMoveList, score;
    var bestScore = 0;
    ...
    while (open.length > 0) {
        ...
        state = obj.state;
        //console.log("Opening state " + state.toString());

        if (visited.indexOf(AI.hashState(state)) >= 0) {
            continue;
        } else {
            //console.log(AI.hashState(state));
            visited.push(AI.hashState(state));
        }
        ...
        if (AI.isTerminalState(state)) {
            ...
            for (var i = 0; i < successors.length; i++) {
                //console.log("Opening move " + successors[i].state.toString());
                //console.log("Trying move " + successors[i].move.toString());
